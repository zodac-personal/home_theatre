...
`--dangerously-skip-permissions`** against this project, fully isolated from the
host machine.

This lives in `sandbox/` inside the project, so the repo root is the single root
...
- **Only the project directory is mounted** (the repo root → `/work`). No
...
- **The host Docker socket is NOT mounted.** The sandbox runs its *own* nested
  Docker daemon, so any container the project spins up lives and dies *inside*
  the sandbox. Your host daemon — and everything already running on it — is never
  touched. Note the flip side for a compose repo like this one: a `docker compose
  up` in a session brings the stack up **inside the container**, against the
  bind-mounted tree, not on the host.
...
**Nothing is published to the host by default.** This project runs no in-sandbox server, and an unused
published port is only a collision risk against the host's own stack. Set `SANDBOX_PORTS` (see *Reusing
...
command, no PTY needed). Running it does `docker stop <project>-sandbox` (the container name, derived
from the project directory — `home-theatre-sandbox` here), which trips the running launcher's teardown
(below). This is the closest thing to a Stop button.
...
1. `git submodule update --init` — only if the project declares a submodule and it
   is uninitialised.
...
4. `.claude/hooks/tests/run-hook-tests.sh` — only if the project has the guards and `jq` is present.
   ...
   actually run them, before any work happens.
...
image than it needs, so trim the `COPY --from=jdk` / `--from=maven` stages if that matters. **This repo
is one of those**: it uses none of the JVM/Node/Playwright half, and carries it only because the image
is already built and trimming has a rebuild cost of its own.
...
```bash
docker volume rm home-theatre-sandbox-{claude,docker,m2,pw}
```
...
- The repo's `.env` (and anything else sensitive in the tree, e.g. `storage/`) is
  mounted at `/work`, so the sandbox *can* read it. Move such files out of the
  tree and inject via `-e` if you want zero exposure.
