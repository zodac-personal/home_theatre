...
run() {
  ...
  #
  # The Maven local repository and the Playwright browser cache get named volumes for the same reason
  # the Docker data dir does: without one they live in the container's writable layer, which --rm
  # deletes on teardown, so every fresh sandbox re-downloads the lot. A named volume — rather than a
  # bind to the host's ~/.m2 — keeps the "no $HOME from the host" rule above intact while still
  # persisting across sessions. The image pre-creates /home/dev/.m2 dev-owned so the volume is writable
  # (see the Dockerfile's user-creation block). THIS project uses neither toolchain, so both volumes
  # simply stay empty; they are kept so the launcher still works unchanged when copied elsewhere.
  docker run "${tty[@]}" --rm \
    --name "${CONTAINER}" \
    --cidfile "${CIDFILE}" \
    --privileged \
